...
public class Behaviour_Laptop /* extends AbstractBehaviorDefault implements IGuiHolder<PlayerInventoryGuiData> */ {
     ...
     * }
     * @Override
     * public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int
     * aY, int aZ, byte aSide, float hitX, float hitY, float hitZ) {
     * if (aPlayer instanceof EntityPlayerMP) {
     * ArrayList<String> tList = new ArrayListNoNulls<>();
     * if (aItem.useEnergy(TD.Energy.EU, aStack, LaptopGUI.scan(null, aPlayer, aWorld, aX, aY, aZ, aSide, hitX, hitY,
     * hitZ), aPlayer, aPlayer.inventory, aWorld, aX, aY, aZ, T)) build;
     * return T;
     * }
     ...
     * public static long scan(ModularPanel panel, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ, byte
     * aSide, float aClickX, float aClickY, float aClickZ) {
     ...
     * try {
     * panel.child(IKey.str("Laptop: Scanned " + (aTileEntity instanceof IInventory &&
     * Code.stringValid(((IInventory)aTileEntity).getInventoryName()) ? ((IInventory)aTileEntity).getInventoryName() :
     * aBlock.getUnlocalizedName()) + "  MetaData: " + aMeta).asWidget());
     * } catch(Throwable e) {e.printStackTrace(ERR);}
     * BlockScanningEvent tEvent = new BlockScanningEvent(aWorld, aPlayer, aX, aY, aZ, aSide, 2, aBlock, aTileEntity,
     * rList, aClickX, aClickY, aClickZ);
