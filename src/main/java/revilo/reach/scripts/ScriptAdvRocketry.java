...
public class ScriptAdvRocketry implements IScriptLoader {
	...
	private ItemStack launchpad = make(MD.GC_ADV_ROCKETRY, "launchpad", 1),
			...
			genericSeat = make (MD.GC_ADV_ROCKETRY, "seat", 1),
			engine = make(MD.GC_ADV_ROCKETRY, "rocketmotor", 1),
			advEngine = make(MD.GC_ADV_ROCKETRY, "advRocket", 1),
			fuelTank = make(MD.GC_ADV_ROCKETRY, "fuelTank", 1),
			sawBlade = make(MD.GC_ADV_ROCKETRY, "sawBlade", 1),
			concrete = make(MD.GC_ADV_ROCKETRY, "concrete", 1),
			...
			oxygenScrubber = make(MD.GC_ADV_ROCKETRY, "scrubber", 1),
			lens = make(MD.GC_ADV_ROCKETRY, "lens", 1);
	
	// Items
	private ItemStack wafer = make(MD.GC_ADV_ROCKETRY, "wafer", 1),
			circuitPlate = make(MD.GC_ADV_ROCKETRY, "circuitplate", 1),
			circuitIC = make(MD.GC_ADV_ROCKETRY, "circuitIC", 1),
			miscPart = make(MD.GC_ADV_ROCKETRY, "miscpart", 1),
    ...
    public void loadMachines() {
   		Injector.addRecipe1(T, 16, 64, make(BlocksGT.Concrete, 1, 0), FL.array(DYE_FLUIDS_WATER[DYE_INDEX_Black], DYE_FLUIDS_WATER[DYE_INDEX_Yellow]), NF, launchpad);
    	Injector.addRecipe1(T, 16, 64, make(BlocksGT.Concrete, 1, 0), FL.array(DYE_FLUIDS_FLOWER[DYE_INDEX_Black], DYE_FLUIDS_FLOWER[DYE_INDEX_Yellow]), NF, launchpad);
    	Injector.addRecipe1(T, 16, 64, make(BlocksGT.Concrete, 1, 0), FL.array(DYE_FLUIDS_CHEMICAL[DYE_INDEX_Black], DYE_FLUIDS_CHEMICAL[DYE_INDEX_Yellow]), NF, launchpad);
    	...
    	Welder.addRecipeX(T, 16, 512, array(tag(0), plateCurved.mat(MT.StainlessSteel, 4), pipeSmall.mat(MT.Al, 2)), engine);
